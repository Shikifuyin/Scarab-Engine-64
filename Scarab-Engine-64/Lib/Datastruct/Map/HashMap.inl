...
template<typename Key, typename T>
inline Bool HashMap<Key,T>::IsCreated() const {
    return ( m_arrTable != NULL );
}
template<typename Key, typename T>
...
template<typename Key, typename T>
inline SizeT HashMap<Key,T>::MemorySize() const {
    Assert( m_arrTable != NULL );
...
inline UInt _Hash_Wrapper( const Key & rKey ) {
    return (UInt)( HashFn->Hash( (const Byte *)(&rKey), sizeof(Key) ) );
}
template<>
inline UInt _Hash_Wrapper<GChar*>( GChar * const & rKey ) {
    return (UInt)( HashFn->Hash( rKey ) );
}
template<>
inline UInt _Hash_Wrapper<const GChar*>( const GChar * const & rKey ) {
    return (UInt)( HashFn->Hash( rKey ) );
}
