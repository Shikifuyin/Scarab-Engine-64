...
public:
    ...
    public:
	    ...
	    inline Iterator & operator=( const Iterator & rhs ) { m_itNode = rhs.m_itNode; return (*this); }

	    inline Bool operator==( const Iterator & rhs ) const { return ( m_itNode == rhs.m_itNode); }
	    inline Bool operator!=( const Iterator & rhs ) const { return ( m_itNode != rhs.m_itNode); }
	    ...
	    inline Iterator Next() const {
            Assert( !( m_itNode.IsNull() ) );
            return _Make( m_itNode.Next() );
        }
	    inline Iterator Previous() const {
            Assert( !( m_itNode.IsNull() ) );
            return _Make( m_itNode.Previous() );
        }
    ...
    private:
        ...
        inline static Iterator _Make( typename RedBlackTree<TreeMapNode>::Iterator itNode ) {
            Iterator it;
            it.m_itNode = itNode;
            return it;
        }
...
public:
    ...
    inline virtual SizeT MemorySize() const;
