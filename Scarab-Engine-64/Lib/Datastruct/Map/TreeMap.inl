...
template<typename Key, typename T>
inline Void TreeMap<Key,T>::UseMemory( MemoryContextID iMemoryContextID, MemoryAllocatorID iAllocatorID ) {
    Datastruct::UseMemory( iMemoryContextID, iAllocatorID );
    m_rbTreeMap.UseMemory( iMemoryContextID, iAllocatorID );
}

template<typename Key, typename T>
inline Void TreeMap<Key,T>::SetComparator( Comparator pfCompare, Void * pUserData ) {
...
template<typename Key, typename T>
inline Bool TreeMap<Key,T>::IsCreated() const {
    return m_rbTreeMap.IsCreated();
}
template<typename Key, typename T>
...
template<typename Key, typename T>
inline SizeT TreeMap<Key,T>::MemorySize() const {
...
{
    typename RedBlackTree<TreeMapNode>::Iterator itFoundNode;
...
inline typename TreeMap<Key,T>::Iterator TreeMap<Key,T>::Begin() const {
    return Iterator::_Make( m_rbTreeMap.Begin() );
}
template<typename Key, typename T>
inline typename TreeMap<Key,T>::Iterator TreeMap<Key,T>::End() const {
    return Iterator::_Make( m_rbTreeMap.End() );
}
template<typename Key, typename T>
typename TreeMap<Key,T>::Iterator TreeMap<Key,T>::Get( const Key & rKey ) const
{
    ...
    return Iterator::_Make( itFoundNode );
}
...
Bool TreeMap<Key,T>::Insert( const Key & rKey, const T & rItem, Iterator * outItem )
{
    ...
    if ( bFound ) {
        ...
        if ( outItem != NULL )
            *outItem = Iterator::_Make( itFoundNode );
        return false;
    }
