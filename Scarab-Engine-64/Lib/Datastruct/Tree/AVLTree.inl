...
template<typename T>
inline Bool AVLTree<T>::IsCreated() const {
    return ( m_iNodeCount != INVALID_OFFSET );
}
template<typename T>
...
template<typename T>
inline SizeT AVLTree<T>::MemorySize() const {
    Assert( m_iNodeCount != INVALID_OFFSET );
...
template<typename T>
inline Bool AVLTree<T>::Insert( const T & rItem ) {
    Assert( m_iNodeCount != INVALID_OFFSET );
    return _rec_Insert( &m_pRoot, m_iHeightChange, rItem );
}
template<typename T>
    ...
    if ( cmp != 0 ) {
        ...
        if ( ((*ppNode)->pChild[AVLTREE_CHILD_LEFT] == NULL) &&
            ...
            *ppNode = NULL;
            ...
            Delete( pRemove, m_iAllocatorID, m_iMemoryContextID );
...
Void AVLTree<T>::_rec_Destroy( AVLNode * pNode )
{
    ...
    Delete( pNode, m_iAllocatorID, m_iMemoryContextID );
}
