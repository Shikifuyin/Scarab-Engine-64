...
Void RandomGenerator::LecuyerSeed( DWord dwSeed, UInt iWarmUp )
{
    if ( dwSeed < 1 )
	    dwSeed = 1;
    m_iLecuyerSeed1 = (signed)dwSeed;
    ...
    // Warmup
    for( UInt j = 0; j < iWarmUp; ++j )
        Lecuyer();
}
DWord RandomGenerator::Lecuyer()
{
    Int iK;
    ...
    return (DWord)( m_iLecuyerCurrent - 1 );
}

Void RandomGenerator::KnuthSeed( DWord dwSeed, UInt iWarmUp )
{
    Int i, j;

    Int iSubstract = ( 161803398 + (Int)dwSeed );
	if ( iSubstract < 0 )
...
DWord RandomGenerator::Knuth()
{
    if ( ++m_iKnuthNext >= sm_iKnuthTableSize )
	    m_iKnuthNext = 1;
    if ( ++m_iKnuthNextP >= sm_iKnuthTableSize )
	    m_iKnuthNextP = 1;
    Int iSubstract = ( m_arrKnuthTable[m_iKnuthNext] - m_arrKnuthTable[m_iKnuthNextP] );
    if ( iSubstract < 0 )
	    iSubstract += sm_iKnuthBig;
    m_arrKnuthTable[m_iKnuthNext] = iSubstract;
    return (DWord)iSubstract;
}
...
Void RandomGenerator::RanRotSeed( UInt64 iSeed, UInt iWarmUp )
{
    ...
    // Init
    UInt iSeed1 = (iSeed & 0x00000000ffffffffui64);
    UInt iSeed2 = (iSeed >> 32);
    for( i = 0; i < sm_iRanRotTableSize; ++i ) {
...
{
    DWord iZ = BitRotL( m_arrRanRotTable[m_iRanRotP1][0], 19 ) + m_arrRanRotTable[m_iRanRotP2][0];
    DWord iY = BitRotL( m_arrRanRotTable[m_iRanRotP1][1], 27 ) + m_arrRanRotTable[m_iRanRotP2][1];
...
Void RandomGenerator::MersenneTwisterSeed( DWord dwSeed, UInt iWarmUp )
{
    Int i;

    DWord iKnuthVal;
    m_arrMersTwistTable[sm_iMersTwistStateSize - 1] = dwSeed;
...
DWord RandomGenerator::MersenneTwister()
{
    if ( m_iMersTwistCurrent <= 0 )
        _MT_RefreshTable();
    DWord iValue = m_arrMersTwistTable[--m_iMersTwistCurrent];
    iValue ^= (iValue >> 11);
...
Void RandomGenerator::MotherOfAll32Seed( DWord dwSeed, UInt iWarmUp )
{
    UInt i;

    // init
    for( i = 0; i < 5; ++i ) {
        dwSeed = dwSeed * 29943829 - 1;
        m_arrMOA32Table[i] = dwSeed;
    }
...
DWord RandomGenerator::MotherOfAll32()
{
    // Summing
    ...
    m_arrMOA32Table[4] = (DWord)(iMWC >> 32); // Carry
    m_arrMOA32Table[0] = (DWord)(iMWC & 0x00000000ffffffffui64); // Random
    return m_arrMOA32Table[0];
}
Void RandomGenerator::MotherOfAll16Seed( DWord dwSeed, UInt iWarmUp )
{
    UInt i;

    // init
    UShort iSh = (UShort)(dwSeed & 0x0000ffff);
    dwSeed = (dwSeed & 0x7fffffff);
    for( i = 0; i < 10; ++i ) {
        dwSeed = 30903ul * iSh + (dwSeed >> 16);
        iSh = (UShort)(dwSeed & 0x0000ffff);
...
DWord RandomGenerator::MotherOfAll16()
{
    // History
    for( Int i = 9; i >= 2; --i ) {
    ...
    // Summing
    DWord iMWC1 = 1941ul * m_arrMOA16Table1[2] + 1860ul * m_arrMOA16Table1[3] +
                  ...
                  m_arrMOA16Table1[0];
    DWord iMWC2 = 1111ul * m_arrMOA16Table2[2] + 2222ul * m_arrMOA16Table2[3] +
    ...
    return ( ((DWord)m_arrMOA16Table1[1]) << 16 ) | ((DWord)m_arrMOA16Table2[1]);
}

Void RandomGenerator::CompMWCSeed( DWord dwSeed, UInt iWarmUp )
{
    UInt i;
...
DWord RandomGenerator::CompMWC()
{
    if ( ++m_iCMWCCurrent >= sm_iCMWCTableSize )
        m_iCMWCCurrent = 0;
    UInt64 iT = 18782ui64 * m_arrCMWCTable[m_iCMWCCurrent] + m_iCMWCCarry;
    m_iCMWCCarry = (DWord)(iT >> 32);
    DWord iX = (DWord)(iT & 0x00000000ffffffffui64) + m_iCMWCCarry;
    if ( iX < m_iCMWCCarry ) {
        ++iX; ++m_iCMWCCarry;
    }
...
Void RandomGenerator::XorShift128Seed( DWord dwSeed, UInt iWarmUp )
{
    // Init
    m_iXorShiftX = dwSeed;
    ...
    // Warmup
    for( UInt i = 0; i < iWarmUp; ++i )
        XorShift128();
}
DWord RandomGenerator::XorShift128()
{
    DWord iT = m_iXorShiftX ^ (m_iXorShiftX << 11);
...
Void RandomGenerator::XorShiftWeylSeed( DWord dwSeed, UInt iWarmUp )
{
    // Init
    m_iXorShiftX = dwSeed;
    m_iXorShiftY = 362436069;
...
DWord RandomGenerator::XorShiftWeyl()
{
    DWord iT = m_iXorShiftX ^ (m_iXorShiftX >> 2);
    m_iXorShiftX = m_iXorShiftY;
    m_iXorShiftY = m_iXorShiftZ;
    m_iXorShiftZ = m_iXorShiftW;
...
Void RandomGenerator::_MT_RefreshTable()
{
    ...
    DWord iScratch1, iScratch2;
    DWord * pState = m_arrMersTwistTable + (sm_iMersTwistStateSize - 1);
    iScratch1 = *pState;
