...
#define MatchFn PatternMatching::GetInstance()
...
public:
    inline static PatternMatching * GetInstance();
...
public:
    ...
    // You must ensure symmetric calls ...
    inline Void EnterChain();
    inline Void LeaveChain();

    // Main Search Method : Default is Raita
    inline UInt Search( const GChar * strText, UInt iLengthT, const GChar * strPattern, UInt iLengthP = 1 );
    ...
    //
    // NOTE : Maxime Crochemore gave me a bad text-algo exam grade once !
    //        Still glad to have one of my elder teacher's name in my code !
    //        As a vengeance I'll just mention his algo just has one of the poorest average
