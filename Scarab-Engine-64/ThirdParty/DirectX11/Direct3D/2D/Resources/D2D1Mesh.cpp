...
Void D2D1Mesh::AddTriangles( const D2D1Triangle * arrTriangles, UInt iTriangleCount )
{
    ...
    ((ID2D1TessellationSink*)m_pTessellationSink)->AddTriangles( (const D2D1_TRIANGLE *)arrTriangles, iTriangleCount );
}
...
{
    m_pMesh = NULL;
    HRESULT hRes = ((ID2D1RenderTarget*)(m_pContext2D->m_pD2D1RenderingContext))->CreateMesh( (ID2D1Mesh**)&m_pMesh );
...
Void D2D1Mesh::_NakedDestroy()
{
    ...
    ((ID2D1Mesh*)m_pMesh)->Release();
    m_pMesh = NULL;
}
Void D2D1Mesh::_NakedOpen()
{
    m_pTessellationSink = NULL;
    HRESULT hRes = ((ID2D1Mesh*)m_pMesh)->Open( (ID2D1TessellationSink**)&m_pTessellationSink );
    DebugAssert( hRes == S_OK && m_pTessellationSink != NULL );
}
Void D2D1Mesh::_NakedClose()
{
    HRESULT hRes = ((ID2D1TessellationSink*)m_pTessellationSink)->Close();
    DebugAssert( hRes == S_OK );

    ((ID2D1TessellationSink*)m_pTessellationSink)->Release();
    m_pTessellationSink = NULL;
}
