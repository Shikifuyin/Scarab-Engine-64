...
Void D2D1FontCollection::Create()
{
    ...
    m_pFontCollection = NULL;
    HRESULT hRes = ((IDWriteFactory*)(D2D1RenderingContext::sm_pDWFactory))->GetSystemFontCollection( (IDWriteFontCollection**)&m_pFontCollection );
...
Void D2D1FontCollection::Destroy()
{
    ...
    ((IDWriteFontCollection*)m_pFontCollection)->Release();
    m_pFontCollection = NULL;
}
...
UInt D2D1FontCollection::GetFontFamilyCount() const
{
    ...
    return ((IDWriteFontCollection*)m_pFontCollection)->GetFontFamilyCount();
}
Void D2D1FontCollection::GetFontFamily( D2D1FontFamily * outFontFamily, UInt iIndex ) const
{
    DebugAssert( IsCreated() );
    DebugAssert( iIndex < ((IDWriteFontCollection*)m_pFontCollection)->GetFontFamilyCount() );

    IDWriteFontFamily * pFontFamily = NULL;
    HRESULT hRes = ((IDWriteFontCollection*)m_pFontCollection)->GetFontFamily( iIndex, &pFontFamily );
    DebugAssert( hRes == S_OK && pFontFamily != NULL );

    if ( outFontFamily->m_pFontFamily != NULL )
        ((IDWriteFontFamily*)(outFontFamily->m_pFontFamily))->Release();
...
Bool D2D1FontCollection::FindFontFamily( UInt * outIndex, const GChar * strFontFamilyName ) const
{
    ...
    BOOL bExists = FALSE;
    HRESULT hRes = ((IDWriteFontCollection*)m_pFontCollection)->FindFamilyName( strFontFamilyName, outIndex, &bExists );
...
Void D2D1FontCollection::EnumerateFontFamilies( GChar ** outFamilyNames, UInt * outNameLengths, UInt iMaxNames, const GChar * strLocaleName ) const
{
    ...
    // Get family count
    UInt iFamilyCount = ((IDWriteFontCollection*)m_pFontCollection)->GetFontFamilyCount();
...
Void D2D1FontFamily::Destroy()
{
    ...
    ((IDWriteFontFamily*)m_pFontFamily)->Release();
    m_pFontFamily = NULL;
}
...
Bool D2D1FontFamily::GetFamilyName( GChar * outFamilyName, UInt * pNameLength, const GChar * strLocaleName ) const
{
    ...
    HRESULT hRes = ((IDWriteFontFamily*)m_pFontFamily)->GetFamilyNames( &pNameStrings );
...
Void D2D1FontFamily::GetFontCollection( D2D1FontCollection * outFontCollection ) const
{
    ...
    HRESULT hRes = ((IDWriteFontFamily*)m_pFontFamily)->GetFontCollection( &pFontCollection );
    DebugAssert( hRes == S_OK && pFontCollection != NULL );

    if ( outFontCollection->m_pFontCollection != NULL )
        ((IDWriteFontCollection*)(outFontCollection->m_pFontCollection))->Release();
...
UInt D2D1FontFamily::GetFontCount() const
{
    ...
    return ((IDWriteFontFamily*)m_pFontFamily)->GetFontCount();
}
Void D2D1FontFamily::GetFont( D2D1Font * outFont, UInt iIndex ) const
{
    DebugAssert( IsCreated() );
    DebugAssert( iIndex < ((IDWriteFontFamily*)m_pFontFamily)->GetFontCount() );

    IDWriteFont * pFont = NULL;
    HRESULT hRes = ((IDWriteFontFamily*)m_pFontFamily)->GetFont( iIndex, &pFont );
    DebugAssert( hRes == S_OK && pFont != NULL );

    if ( outFont->m_pFont != NULL )
        ((IDWriteFont*)(outFont->m_pFont))->Release();
...
Void D2D1FontFamily::MatchClosestFont( D2D1Font * outFont, D2D1FontWeight iWeight, D2D1FontStyle iStyle, D2D1FontStretch iStretch ) const
{
    ...
    IDWriteFont * pFont = NULL;
    HRESULT hRes = ((IDWriteFontFamily*)m_pFontFamily)->GetFirstMatchingFont( (DWRITE_FONT_WEIGHT)(D2D1FontWeightToD2D1[iWeight]),
                                                                              (DWRITE_FONT_STRETCH)(D2D1FontStretchToD2D1[iStretch]),
                                                                              (DWRITE_FONT_STYLE)(D2D1FontStyleToD2D1[iStyle]), &pFont );
    DebugAssert( hRes == S_OK && pFont != NULL );
...
Void D2D1FontFamily::MatchFonts( D2D1FontList * outFontList, D2D1FontWeight iWeight, D2D1FontStyle iStyle, D2D1FontStretch iStretch ) const
{
    ...
    HRESULT hRes = ((IDWriteFontFamily*)m_pFontFamily)->GetMatchingFonts( (DWRITE_FONT_WEIGHT)(D2D1FontWeightToD2D1[iWeight]),
                                                                          (DWRITE_FONT_STRETCH)(D2D1FontStretchToD2D1[iStretch]),
                                                                          (DWRITE_FONT_STYLE)(D2D1FontStyleToD2D1[iStyle]), &pFontList );
    DebugAssert( hRes == S_OK && pFontList != NULL );

    if ( outFontList->m_pFontList != NULL )
        ((IDWriteFontList*)(outFontList->m_pFontList))->Release();
...
Bool D2D1Font::IsSymbolFont() const
{
    ...
    return ( ((IDWriteFont*)m_pFont)->IsSymbolFont() != FALSE );
}
Bool D2D1Font::HasCharacter( UInt iCharacter ) const
{
    ...
    BOOL bExists = FALSE;
    HRESULT hRes = ((IDWriteFont*)m_pFont)->HasCharacter( iCharacter, &bExists );
    DebugAssert( hRes == S_OK );
...
Bool D2D1Font::GetFaceName( GChar * outFaceName, UInt * pNameLength, const GChar * strLocaleName ) const
{
    ...
    IDWriteLocalizedStrings * pNameStrings = NULL;
    HRESULT hRes = ((IDWriteFont*)m_pFont)->GetFaceNames( &pNameStrings );
    DebugAssert( hRes == S_OK && pNameStrings != NULL );
...
Bool D2D1Font::GetInfoString( GChar * outInfoString, UInt * pStringLength, D2D1FontInfoStringID idInfoString, const GChar * strLocaleName ) const
{
    ...
    BOOL bExists = FALSE;
    IDWriteLocalizedStrings * pNameStrings = NULL;
    HRESULT hRes = ((IDWriteFont*)m_pFont)->GetInformationalStrings( (DWRITE_INFORMATIONAL_STRING_ID)(D2D1FontInfoStringIDToD2D1[idInfoString]), &pNameStrings, &bExists );
    DebugAssert( hRes == S_OK );
...
D2D1FontWeight D2D1Font::GetWeight() const
{
    ...
    return D2D1FontWeightFromD2D1( ((IDWriteFont*)m_pFont)->GetWeight() );
}
D2D1FontStyle D2D1Font::GetStyle() const
{
    DebugAssert( IsCreated() );

    return D2D1FontStyleFromD2D1[ ((IDWriteFont*)m_pFont)->GetStyle() ];
}
D2D1FontStretch D2D1Font::GetStretch() const
{
    DebugAssert( IsCreated() );

    return D2D1FontStretchFromD2D1[ ((IDWriteFont*)m_pFont)->GetStretch() ];
}
...
D2D1FontSimulation D2D1Font::GetSimulation() const
{
    ...
    return D2D1FontSimulationFromD2D1[ ((IDWriteFont*)m_pFont)->GetSimulations() ];
}
Void D2D1Font::GetMetrics( D2D1FontMetrics * outFontMetrics ) const
{
    ...
    ((IDWriteFont*)m_pFont)->GetMetrics( &hMetrics );
...
Void D2D1Font::GetFontFamily( D2D1FontFamily * outFontFamily ) const
{
    ...
    IDWriteFontFamily * pFontFamily = NULL;
    HRESULT hRes = ((IDWriteFont*)m_pFont)->GetFontFamily( &pFontFamily );
    DebugAssert( hRes == S_OK && pFontFamily != NULL );
...
Void D2D1Font::CreateFontFace( D2D1FontFace * outFontFace ) const
{
    ...
    HRESULT hRes = ((IDWriteFont*)m_pFont)->CreateFontFace( &pFontFace );
    DebugAssert( hRes == S_OK && pFontFace != NULL );

    if ( outFontFace->m_pFontFace != NULL )
        ((IDWriteFontFace*)(outFontFace->m_pFontFace))->Release();
...
Void D2D1FontList::Destroy()
{
    ...
    ((IDWriteFontList*)m_pFontList)->Release();
    m_pFontList = NULL;
}
...
Void D2D1FontList::GetFontCollection( D2D1FontCollection * outFontCollection ) const
{
    ...
    IDWriteFontCollection * pFontCollection = NULL;
    HRESULT hRes = ((IDWriteFontList*)m_pFontList)->GetFontCollection( &pFontCollection );
    DebugAssert( hRes == S_OK && pFontCollection != NULL );
...
UInt D2D1FontList::GetFontCount() const
{
    ...
    return ((IDWriteFontList*)m_pFontList)->GetFontCount();
}
Void D2D1FontList::GetFont( D2D1Font * outFont, UInt iIndex ) const
{
    DebugAssert( IsCreated() );
    DebugAssert( iIndex < ((IDWriteFontList*)m_pFontList)->GetFontCount() );

    IDWriteFont * pFont = NULL;
    HRESULT hRes = ((IDWriteFontList*)m_pFontList)->GetFont( iIndex, &pFont );
    DebugAssert( hRes == S_OK && pFont != NULL );
...
Void D2D1FontFace::Destroy()
{
    ...
    ((IDWriteFontFace*)m_pFontFace)->Release();
    m_pFontFace = NULL;
}
...
Bool D2D1FontFace::IsSymbolFont() const
{
    ...
    return ( ((IDWriteFontFace*)m_pFontFace)->IsSymbolFont() != FALSE );
}
...
UInt D2D1FontFace::GetIndex() const
{
    ...
    return ((IDWriteFontFace*)m_pFontFace)->GetIndex();
}
D2D1FontFaceType D2D1FontFace::GetType() const
{
    DebugAssert( IsCreated() );

    return D2D1FontFaceTypeFromD2D1[ ((IDWriteFontFace*)m_pFontFace)->GetType() ];
}
...
D2D1FontSimulation D2D1FontFace::GetSimulation() const
{
    ...
    return D2D1FontSimulationFromD2D1[ ((IDWriteFontFace*)m_pFontFace)->GetSimulations() ];
}
Void D2D1FontFace::GetMetrics( D2D1FontMetrics * outFontMetrics ) const
{
    ...
    DWRITE_FONT_METRICS hMetrics;
    ((IDWriteFontFace*)m_pFontFace)->GetMetrics( &hMetrics );
...
Void D2D1FontFace::GetGdiCompatibleMetrics( D2D1FontMetrics * outFontMetrics, Float fSizeDIP, Float fPixelsPerDIP, const D2D1Matrix32 * pTransform ) const
{
    ...
    DWRITE_FONT_METRICS hMetrics;
    HRESULT hRes = ((IDWriteFontFace*)m_pFontFace)->GetGdiCompatibleMetrics( fSizeDIP, fPixelsPerDIP, (const DWRITE_MATRIX *)pTransform, &hMetrics );
    DebugAssert( hRes == S_OK );
...
Void D2D1FontFace::GetRecommendedRenderingMode( D2D1TextRenderingMode * outMode, Float fSizeDIP, Float fPixelsPerDIP, D2D1TextMeasuringMode iMeasuringMode, const D2D1TextRenderState * pTextRenderState ) const
{
    ...
    HRESULT hRes = ((IDWriteFontFace*)m_pFontFace)->GetRecommendedRenderingMode( fSizeDIP, fPixelsPerDIP, (DWRITE_MEASURING_MODE)(D2D1TextMeasuringModeToD2D1[iMeasuringMode]),
                                                                                 (IDWriteRenderingParams*)(pTextRenderState->m_pTextRenderingParams), &iTmpMode );
    DebugAssert( hRes == S_OK );
...
UInt D2D1FontFace::GetGlyphCount() const
{
    ...
    return (UInt)( ((IDWriteFontFace*)m_pFontFace)->GetGlyphCount() );
}
Void D2D1FontFace::GetGlyphIndices( UShort * outGlyphIndices, const UInt * arrCodePointsUSC4, UInt iCodePointCount ) const
{
    ...
    HRESULT hRes = ((IDWriteFontFace*)m_pFontFace)->GetGlyphIndicesW( arrCodePointsUSC4, iCodePointCount, outGlyphIndices );
#else
    HRESULT hRes = ((IDWriteFontFace*)m_pFontFace)->GetGlyphIndicesA( arrCodePointsUSC4, iCodePointCount, outGlyphIndices );
#endif
    DebugAssert( hRes == S_OK );
}
...
Void D2D1FontFace::GetDesignGlyphMetrics( D2D1FontGlyphMetrics * outGlyphMetrics, const UShort * arrGlyphIndices, UInt iGlyphCount, Bool bIsSideways ) const
{
    ...
    HRESULT hRes = ((IDWriteFontFace*)m_pFontFace)->GetDesignGlyphMetrics( arrGlyphIndices, iGlyphCount, &hGlyphMetrics, bIsSideways ? TRUE : FALSE );
    DebugAssert( hRes == S_OK );

    outGlyphMetrics->ConvertFrom( &hGlyphMetrics );
}
Void D2D1FontFace::GetGdiCompatibleGlyphMetrics( D2D1FontGlyphMetrics * outGlyphMetrics, const UShort * arrGlyphIndices, UInt iGlyphCount, Bool bIsSideways,
                                                 Float fSizeDIP, Float fPixelsPerDIP, Bool bUseGdiNatural, const D2D1Matrix32 * pTransform ) const
{
    DebugAssert( IsCreated() );

    DWRITE_GLYPH_METRICS hGlyphMetrics;
    HRESULT hRes = ((IDWriteFontFace*)m_pFontFace)->GetGdiCompatibleGlyphMetrics( fSizeDIP, fPixelsPerDIP, (const DWRITE_MATRIX *)pTransform, bUseGdiNatural ? TRUE : FALSE,
                                       ...
                                       const Float * arrGlyphAdvances, const D2D1FontGlyphOffset * arrGlyphOffsets ) const
{
    ...
    HRESULT hRes = ((IDWriteFontFace*)m_pFontFace)->GetGlyphRunOutline( fSizeDIP, arrGlyphIndices, arrGlyphAdvances, (const DWRITE_GLYPH_OFFSET *)arrGlyphOffsets, iGlyphCount,
                                                                        bIsSideways ? TRUE : FALSE, bIsRightToLeft ? TRUE : FALSE, (ID2D1GeometrySink*)(outGeometry->m_pGeometrySink) );
    DebugAssert( hRes == S_OK );
}
...
Void D2D1TextFormat::Create( const GChar * strLocaleName, const GChar * strFontFamilyName, D2D1FontWeight iWeight, D2D1FontStyle iStyle, D2D1FontStretch iStretch, Float fSize, D2D1FontCollection * pFontCollection )
{
    ...
    m_pTextFormat = NULL;
    HRESULT hRes = ((IDWriteFactory*)(D2D1RenderingContext::sm_pDWFactory))->CreateTextFormat( strFontFamilyName, (pFontCollection != NULL) ? (IDWriteFontCollection*)(pFontCollection->m_pFontCollection) : NULL,
                                                                                               (DWRITE_FONT_WEIGHT)(D2D1FontWeightToD2D1[iWeight]),
                                                                                               (DWRITE_FONT_STYLE)(D2D1FontStyleToD2D1[iStyle]),
                                                                                               (DWRITE_FONT_STRETCH)(D2D1FontStretchToD2D1[iStretch]),
                                                                                               fSize, strLocaleName, (IDWriteTextFormat**)&m_pTextFormat );
...
Void D2D1TextFormat::Destroy()
{
    ...
    ((IDWriteTextFormat*)m_pTextFormat)->Release();
    m_pTextFormat = NULL;
}
...
Void D2D1TextFormat::GetFontCollection( D2D1FontCollection * outFontCollection ) const
{
    ...
    IDWriteFontCollection * pFontCollection = NULL;
    HRESULT hRes = ((IDWriteTextFormat*)m_pTextFormat)->GetFontCollection( &pFontCollection );
    DebugAssert( hRes == S_OK && pFontCollection != NULL );
...
UInt D2D1TextFormat::GetLocaleNameLength() const
{
    ...
    return ((IDWriteTextFormat*)m_pTextFormat)->GetLocaleNameLength();
}
Void D2D1TextFormat::GetLocaleName( GChar * outLocaleName, UInt iMaxLength ) const
{
    DebugAssert( IsCreated() );

    HRESULT hRes = ((IDWriteTextFormat*)m_pTextFormat)->GetLocaleName( outLocaleName, iMaxLength + 1 );
    DebugAssert( hRes == S_OK );
}
UInt D2D1TextFormat::GetFontFamilyNameLength() const
{
    ...
    return ((IDWriteTextFormat*)m_pTextFormat)->GetFontFamilyNameLength();
}
Void D2D1TextFormat::GetFontFamilyName( GChar * outFontFamilyName, UInt iMaxLength ) const
{
    DebugAssert( IsCreated() );

    HRESULT hRes = ((IDWriteTextFormat*)m_pTextFormat)->GetFontFamilyName( outFontFamilyName, iMaxLength + 1 );
    DebugAssert( hRes == S_OK );
}
...
D2D1FontWeight D2D1TextFormat::GetFontWeight() const
{
    ...
    return D2D1FontWeightFromD2D1( ((IDWriteTextFormat*)m_pTextFormat)->GetFontWeight() );
}
D2D1FontStyle D2D1TextFormat::GetFontStyle() const
{
    DebugAssert( IsCreated() );

    return D2D1FontStyleFromD2D1[ ((IDWriteTextFormat*)m_pTextFormat)->GetFontStyle() ];
}
D2D1FontStretch D2D1TextFormat::GetFontStretch() const
{
    DebugAssert( IsCreated() );

    return D2D1FontStretchFromD2D1[ ((IDWriteTextFormat*)m_pTextFormat)->GetFontStretch() ];
}
Float D2D1TextFormat::GetFontSize() const
{
    DebugAssert( IsCreated() );

    return ((IDWriteTextFormat*)m_pTextFormat)->GetFontSize();
}
...
D2D1TextFlowDirection D2D1TextFormat::GetFlowDirection() const
{
    ...
    return D2D1TextFlowDirectionFromD2D1[ ((IDWriteTextFormat*)m_pTextFormat)->GetFlowDirection() ];
}
Void D2D1TextFormat::SetFlowDirection( D2D1TextFlowDirection iFlowDirection )
{
    DebugAssert( IsCreated() );

    HRESULT hRes = ((IDWriteTextFormat*)m_pTextFormat)->SetFlowDirection( (DWRITE_FLOW_DIRECTION)(D2D1TextFlowDirectionToD2D1[iFlowDirection]) );
    DebugAssert( hRes == S_OK );
}
...
D2D1TextReadingDirection D2D1TextFormat::GetReadingDirection() const
{
    ...
    return D2D1TextReadingDirectionFromD2D1[ ((IDWriteTextFormat*)m_pTextFormat)->GetReadingDirection() ];
}
Void D2D1TextFormat::SetReadingDirection( D2D1TextReadingDirection iReadingDirection )
{
    DebugAssert( IsCreated() );

    HRESULT hRes = ((IDWriteTextFormat*)m_pTextFormat)->SetReadingDirection( (DWRITE_READING_DIRECTION)(D2D1TextReadingDirectionToD2D1[iReadingDirection]) );
    DebugAssert( hRes == S_OK );
}
...
D2D1TextLineSpacingMethod D2D1TextFormat::GetLineSpacing( Float * outLineSpacing, Float * outBaseline ) const
{
    ...
    Float fLineSpacing;
    Float fBaseline;
    HRESULT hRes = ((IDWriteTextFormat*)m_pTextFormat)->GetLineSpacing( &iMethod, &fLineSpacing, &fBaseline );
    DebugAssert( hRes == S_OK );
...
Void D2D1TextFormat::SetLineSpacing( D2D1TextLineSpacingMethod iLineSpacingMethod, Float fLineSpacing, Float fBaseline )
{
    ...
    HRESULT hRes = ((IDWriteTextFormat*)m_pTextFormat)->SetLineSpacing( (DWRITE_LINE_SPACING_METHOD)(D2D1TextLineSpacingMethodToD2D1[iLineSpacingMethod]), fLineSpacing, fBaseline );
    DebugAssert( hRes == S_OK );
}
...
D2D1TextAlignment D2D1TextFormat::GetTextAlignment() const
{
    ...
    return D2D1TextAlignmentFromD2D1[ ((IDWriteTextFormat*)m_pTextFormat)->GetTextAlignment() ];
}
Void D2D1TextFormat::SetTextAlignment( D2D1TextAlignment iTextAlignment )
{
    DebugAssert( IsCreated() );

    HRESULT hRes = ((IDWriteTextFormat*)m_pTextFormat)->SetTextAlignment( (DWRITE_TEXT_ALIGNMENT)(D2D1TextAlignmentToD2D1[iTextAlignment]) );
    DebugAssert( hRes == S_OK );
}
...
D2D1TextParagraphAlignment D2D1TextFormat::GetParagraphAlignment() const
{
    ...
    return D2D1TextParagraphAlignmentFromD2D1[ ((IDWriteTextFormat*)m_pTextFormat)->GetParagraphAlignment() ];
}
Void D2D1TextFormat::SetParagraphAlignment( D2D1TextParagraphAlignment iParagraphAlignment )
{
    DebugAssert( IsCreated() );

    HRESULT hRes = ((IDWriteTextFormat*)m_pTextFormat)->SetParagraphAlignment( (DWRITE_PARAGRAPH_ALIGNMENT)(D2D1TextParagraphAlignmentToD2D1[iParagraphAlignment]) );
    DebugAssert( hRes == S_OK );
}
...
Float D2D1TextFormat::GetIncrementalTabStop() const
{
    ...
    return ((IDWriteTextFormat*)m_pTextFormat)->GetIncrementalTabStop();
}
Void D2D1TextFormat::SetIncrementalTabStop( Float fIncrementalTabStop )
{
    DebugAssert( IsCreated() );

    HRESULT hRes = ((IDWriteTextFormat*)m_pTextFormat)->SetIncrementalTabStop( fIncrementalTabStop );
    DebugAssert( hRes == S_OK );
}
...
D2D1TextTrimmingGranularity D2D1TextFormat::GetTrimming( GChar * outDelimiter, UInt * outDelimiterCount ) const
{
    ...
    HRESULT hRes = ((IDWriteTextFormat*)m_pTextFormat)->GetTrimming( &hTrimmingOptions, NULL );
    DebugAssert( hRes == S_OK );
...
Void D2D1TextFormat::SetTrimming( D2D1TextTrimmingGranularity iTrimmingGranularity, GChar chDelimiter, UInt iDelimiterCount )
{
    ...
    DWRITE_TRIMMING hTrimmingOptions;
    hTrimmingOptions.granularity = (DWRITE_TRIMMING_GRANULARITY)( D2D1TextTrimmingGranularityToD2D1[iTrimmingGranularity] );
    hTrimmingOptions.delimiter = (UInt)chDelimiter;
    hTrimmingOptions.delimiterCount = iDelimiterCount;

    HRESULT hRes = ((IDWriteTextFormat*)m_pTextFormat)->SetTrimming( &hTrimmingOptions, NULL );
    DebugAssert( hRes == S_OK );
}
...
D2D1TextWordWrapping D2D1TextFormat::GetWordWrapping() const
{
    ...
    return D2D1TextWordWrappingFromD2D1[ ((IDWriteTextFormat*)m_pTextFormat)->GetWordWrapping() ];
}
Void D2D1TextFormat::SetWordWrapping( D2D1TextWordWrapping iWordWrapping )
{
    DebugAssert( IsCreated() );

    HRESULT hRes = ((IDWriteTextFormat*)m_pTextFormat)->SetWordWrapping( (DWRITE_WORD_WRAPPING)(D2D1TextWordWrappingToD2D1[iWordWrapping]) );
    DebugAssert( hRes == S_OK );
}
...
class D2D1TextInlineObject::_DWriteInlineObject : public IDWriteInlineObject {
    public:
        ...
        virtual HRESULT __stdcall GetBreakConditions( DWRITE_BREAK_CONDITION * outBreakConditionBefore, DWRITE_BREAK_CONDITION * outBreakConditionAfter ) {
            *outBreakConditionBefore = (DWRITE_BREAK_CONDITION)(D2D1TextBreakConditionToD2D1[m_pParentHandler->m_iBreakBefore]);
            *outBreakConditionAfter = (DWRITE_BREAK_CONDITION)(D2D1TextBreakConditionToD2D1[m_pParentHandler->m_iBreakAfter]);
            return S_OK;
        }
...
Void D2D1TextLayout::Create( D2D1TextFormat * pTextFormat, Float fMaxWidth, Float fMaxHeight, const GChar * strText, UInt iTextLength )
{
    ...
    m_pTextLayout = NULL;
    HRESULT hRes = ((IDWriteFactory*)(D2D1RenderingContext::sm_pDWFactory))->CreateTextLayout( strText, iTextLength, (IDWriteTextFormat*)(pTextFormat->m_pTextFormat),
                                                                                               fMaxWidth, fMaxHeight, (IDWriteTextLayout**)&m_pTextLayout );
...
Void D2D1TextLayout::Destroy()
{
    ...
    ((IDWriteTextLayout*)m_pTextLayout)->Release();
    m_pTextLayout = NULL;
}
...
Float D2D1TextLayout::GetMaxWidth() const
{
    ...
    return ((IDWriteTextLayout*)m_pTextLayout)->GetMaxWidth();
}
Void D2D1TextLayout::SetMaxWidth( Float fMaxWidth )
{
    DebugAssert( IsCreated() );

    HRESULT hRes = ((IDWriteTextLayout*)m_pTextLayout)->SetMaxWidth( fMaxWidth );
    DebugAssert( hRes == S_OK );
}
...
Float D2D1TextLayout::GetMaxHeight() const
{
    ...
    return ((IDWriteTextLayout*)m_pTextLayout)->GetMaxHeight();
}
Void D2D1TextLayout::SetMaxHeight( Float fMaxHeight )
{
    DebugAssert( IsCreated() );

    HRESULT hRes = ((IDWriteTextLayout*)m_pTextLayout)->SetMaxHeight( fMaxHeight );
    DebugAssert( hRes == S_OK );
}
...
Void D2D1TextLayout::GetMetrics( D2D1TextMetrics * outTextMetrics ) const
{
    ...
    HRESULT hRes = ((IDWriteTextLayout*)m_pTextLayout)->GetMetrics( &hTextMetrics );
    DebugAssert( hRes == S_OK );
...
Void D2D1TextLayout::GetOverhangMetrics( D2D1TextOverhangMetrics * outOverhangMetrics ) const
{
    ...
    HRESULT hRes = ((IDWriteTextLayout*)m_pTextLayout)->GetOverhangMetrics( &hOverhangMetrics );
    DebugAssert( hRes == S_OK );
...
UInt D2D1TextLayout::GetLineMetricCount() const
{
    ...
    HRESULT hRes = ((IDWriteTextLayout*)m_pTextLayout)->GetLineMetrics( &hLineMetrics, 1, &iLineMetricsCount );
...
Void D2D1TextLayout::GetLineMetrics( D2D1TextLineMetrics * outLineMetrics, UInt iMaxLines ) const
{
    ...
    HRESULT hRes = ((IDWriteTextLayout*)m_pTextLayout)->GetLineMetrics( (DWRITE_LINE_METRICS*)outLineMetrics, iMaxLines, &iMaxLines );
    DebugAssert( hRes == S_OK || hRes == E_NOT_SUFFICIENT_BUFFER );
}
...
UInt D2D1TextLayout::GetClusterMetricCount() const
{
    ...
    HRESULT hRes = ((IDWriteTextLayout*)m_pTextLayout)->GetClusterMetrics( &hClusterMetrics, 1, &iClusterMetricsCount );
    DebugAssert( hRes == S_OK || hRes == E_NOT_SUFFICIENT_BUFFER );
...
Void D2D1TextLayout::GetClusterMetrics( D2D1TextClusterMetrics * outClusterMetrics, UInt iMaxClusters ) const
{
    ...
    HRESULT hRes = ((IDWriteTextLayout*)m_pTextLayout)->GetClusterMetrics( (DWRITE_CLUSTER_METRICS*)outClusterMetrics, iMaxClusters, &iMaxClusters );
    DebugAssert( hRes == S_OK || hRes == E_NOT_SUFFICIENT_BUFFER );
}
...
Float D2D1TextLayout::ComputeMinWidth() const
{
    ...
    HRESULT hRes = ((IDWriteTextLayout*)m_pTextLayout)->DetermineMinWidth( &fMinWidth );
    DebugAssert( hRes == S_OK );
...
Void D2D1TextLayout::HitTestPoint( D2D1TextHitTestMetrics * outHitTestMetrics, Bool * outIsInside, Bool * outIsTrailingHit, Float fX, Float fY ) const
{
    ...
    HRESULT hRes = ((IDWriteTextLayout*)m_pTextLayout)->HitTestPoint( fX, fY, &bIsTrailing, &bIsInside, (DWRITE_HIT_TEST_METRICS*)outHitTestMetrics );
    DebugAssert( hRes == S_OK );
...
Void D2D1TextLayout::HitTestTextPosition( D2D1TextHitTestMetrics * outHitTestMetrics, Float * outX, Float * outY, UInt iPosition, Bool bIsTrailingHit ) const
{
    ...
    HRESULT hRes = ((IDWriteTextLayout*)m_pTextLayout)->HitTestTextPosition( iPosition, (bIsTrailingHit) ? TRUE : FALSE, outX, outY, (DWRITE_HIT_TEST_METRICS*)outHitTestMetrics );
    DebugAssert( hRes == S_OK );
}
...
UInt D2D1TextLayout::HitTestTextRangeCount( const D2D1TextRange * pRange, Float fOriginX, Float fOriginY ) const
{
    ...
    HRESULT hRes = ((IDWriteTextLayout*)m_pTextLayout)->HitTestTextRange( pRange->iPosition, pRange->iLength, fOriginX, fOriginY, NULL, 0, &iHitTestCount );
    DebugAssert( hRes == S_OK || hRes == E_NOT_SUFFICIENT_BUFFER );
...
Void D2D1TextLayout::HitTestTextRange( D2D1TextHitTestMetrics * outHitTestMetrics, UInt iMaxHitTests, const D2D1TextRange * pRange, Float fOriginX, Float fOriginY ) const
{
    ...
    HRESULT hRes = ((IDWriteTextLayout*)m_pTextLayout)->HitTestTextRange( pRange->iPosition, pRange->iLength, fOriginX, fOriginY, (DWRITE_HIT_TEST_METRICS*)outHitTestMetrics, iMaxHitTests, &iMaxHitTests );
    DebugAssert( hRes == S_OK || hRes == E_NOT_SUFFICIENT_BUFFER );
}
...
UInt D2D1TextLayout::GetLocaleNameLength( UInt iPosition, D2D1TextRange * outRange ) const
{
    ...
    UInt iLength = 0;
    HRESULT hRes = ((IDWriteTextLayout*)m_pTextLayout)->GetLocaleNameLength( iPosition, &iLength, (DWRITE_TEXT_RANGE*)outRange );
    DebugAssert( hRes == S_OK );
...
Void D2D1TextLayout::GetLocaleName( GChar * outLocaleName, UInt iNameSize, UInt iPosition, D2D1TextRange * outRange ) const
{
    ...
    HRESULT hRes = ((IDWriteTextLayout*)m_pTextLayout)->GetLocaleName( iPosition, outLocaleName, iNameSize, (DWRITE_TEXT_RANGE*)outRange );
    DebugAssert( hRes == S_OK );
}
Void D2D1TextLayout::SetLocaleName( const GChar * strLocaleName, const D2D1TextRange * pRange )
{
    ...
    HRESULT hRes = ((IDWriteTextLayout*)m_pTextLayout)->SetLocaleName( strLocaleName, *((const DWRITE_TEXT_RANGE *)pRange) );
    DebugAssert( hRes == S_OK );
}
...
Void D2D1TextLayout::GetFontCollection( D2D1FontCollection * outFontCollection, UInt iPosition, D2D1TextRange * outRange ) const
{
    ...
    IDWriteFontCollection * pFontCollection = NULL;
    HRESULT hRes = ((IDWriteTextLayout*)m_pTextLayout)->GetFontCollection( iPosition, &pFontCollection, (DWRITE_TEXT_RANGE*)outRange );
    DebugAssert( hRes == S_OK && pFontCollection != NULL );
...
Void D2D1TextLayout::SetFontCollection( D2D1FontCollection * pFontCollection, const D2D1TextRange * pRange )
{
    ...
    HRESULT hRes = ((IDWriteTextLayout*)m_pTextLayout)->SetFontCollection( (IDWriteFontCollection*)(pFontCollection->m_pFontCollection), *((const DWRITE_TEXT_RANGE *)pRange) );
    DebugAssert( hRes == S_OK );
}
...
UInt D2D1TextLayout::GetFontFamilyNameLength( UInt iPosition, D2D1TextRange * outRange ) const
{
    ...
    UInt iLength = 0;
    HRESULT hRes = ((IDWriteTextLayout*)m_pTextLayout)->GetFontFamilyNameLength( iPosition, &iLength, (DWRITE_TEXT_RANGE*)outRange );
    DebugAssert( hRes == S_OK );
...
Void D2D1TextLayout::GetFontFamilyName( GChar * outFamilyName, UInt iNameSize, UInt iPosition, D2D1TextRange * outRange ) const
{
    ...
    HRESULT hRes = ((IDWriteTextLayout*)m_pTextLayout)->GetFontFamilyName( iPosition, outFamilyName, iNameSize, (DWRITE_TEXT_RANGE*)outRange );
    DebugAssert( hRes == S_OK );
}
Void D2D1TextLayout::SetFontFamilyName( const GChar * strFontFamilyName, const D2D1TextRange * pRange )
{
    ...
    HRESULT hRes = ((IDWriteTextLayout*)m_pTextLayout)->SetFontFamilyName( strFontFamilyName, *((const DWRITE_TEXT_RANGE *)pRange) );
    DebugAssert( hRes == S_OK );
}
...
D2D1FontWeight D2D1TextLayout::GetFontWeight( UInt iPosition, D2D1TextRange * outRange ) const
{
    ...
    HRESULT hRes = ((IDWriteTextLayout*)m_pTextLayout)->GetFontWeight( iPosition, &iWeight, (DWRITE_TEXT_RANGE*)outRange );
    DebugAssert( hRes == S_OK );
...
Void D2D1TextLayout::SetFontWeight( D2D1FontWeight iWeight, const D2D1TextRange * pRange )
{
    ...
    HRESULT hRes = ((IDWriteTextLayout*)m_pTextLayout)->SetFontWeight( (DWRITE_FONT_WEIGHT)(D2D1FontWeightToD2D1[iWeight]), *((const DWRITE_TEXT_RANGE *)pRange) );
    DebugAssert( hRes == S_OK );
}
...
D2D1FontStyle D2D1TextLayout::GetFontStyle( UInt iPosition, D2D1TextRange * outRange ) const
{
    ...
    HRESULT hRes = ((IDWriteTextLayout*)m_pTextLayout)->GetFontStyle( iPosition, &iStyle, (DWRITE_TEXT_RANGE*)outRange );
    DebugAssert( hRes == S_OK );
...
Void D2D1TextLayout::SetFontStyle( D2D1FontStyle iStyle, const D2D1TextRange * pRange )
{
    ...
    HRESULT hRes = ((IDWriteTextLayout*)m_pTextLayout)->SetFontStyle( (DWRITE_FONT_STYLE)(D2D1FontStyleToD2D1[iStyle]), *((const DWRITE_TEXT_RANGE *)pRange) );
    DebugAssert( hRes == S_OK );
}
...
D2D1FontStretch D2D1TextLayout::GetFontStretch( UInt iPosition, D2D1TextRange * outRange ) const
{
    ...
    HRESULT hRes = ((IDWriteTextLayout*)m_pTextLayout)->GetFontStretch( iPosition, &iStretch, (DWRITE_TEXT_RANGE*)outRange );
    DebugAssert( hRes == S_OK );
...
Void D2D1TextLayout::SetFontStretch( D2D1FontStretch iStretch, const D2D1TextRange * pRange )
{
    ...
    HRESULT hRes = ((IDWriteTextLayout*)m_pTextLayout)->SetFontStretch( (DWRITE_FONT_STRETCH)(D2D1FontStretchToD2D1[iStretch]), *((const DWRITE_TEXT_RANGE *)pRange) );
    DebugAssert( hRes == S_OK );
}
...
Float D2D1TextLayout::GetFontSize( UInt iPosition, D2D1TextRange * outRange ) const
{
    ...
    Float fSize;
    HRESULT hRes = ((IDWriteTextLayout*)m_pTextLayout)->GetFontSize( iPosition, &fSize, (DWRITE_TEXT_RANGE*)outRange );
    DebugAssert( hRes == S_OK );
...
Void D2D1TextLayout::SetFontSize( Float fSize, const D2D1TextRange * pRange )
{
    ...
    HRESULT hRes = ((IDWriteTextLayout*)m_pTextLayout)->SetFontSize( fSize, *((const DWRITE_TEXT_RANGE *)pRange) );
    DebugAssert( hRes == S_OK );
}
...
Bool D2D1TextLayout::HasUnderline( UInt iPosition, D2D1TextRange * outRange ) const
{
    ...
    BOOL bHasUnderline;
    HRESULT hRes = ((IDWriteTextLayout*)m_pTextLayout)->GetUnderline( iPosition, &bHasUnderline, (DWRITE_TEXT_RANGE*)outRange );
    DebugAssert( hRes == S_OK );
...
Void D2D1TextLayout::SetUnderline( Bool bUnderline, const D2D1TextRange * pRange )
{
    ...
    HRESULT hRes = ((IDWriteTextLayout*)m_pTextLayout)->SetUnderline( (bUnderline) ? TRUE : FALSE, *((const DWRITE_TEXT_RANGE *)pRange) );
    DebugAssert( hRes == S_OK );
}
...
Bool D2D1TextLayout::HasStrikethrough( UInt iPosition, D2D1TextRange * outRange ) const
{
    ...
    HRESULT hRes = ((IDWriteTextLayout*)m_pTextLayout)->GetStrikethrough( iPosition, &bHasStrikethrough, (DWRITE_TEXT_RANGE*)outRange );
    DebugAssert( hRes == S_OK );
...
Void D2D1TextLayout::SetStrikethrough( Bool bStrikethrough, const D2D1TextRange * pRange )
{
    ...
    HRESULT hRes = ((IDWriteTextLayout*)m_pTextLayout)->SetStrikethrough( (bStrikethrough) ? TRUE : FALSE, *((const DWRITE_TEXT_RANGE *)pRange) );
    DebugAssert( hRes == S_OK );
}
...
Void D2D1TextLayout::GetForegroundBrush( D2D1SolidColorBrush * outBrush, UInt iPosition, D2D1TextRange * outRange ) const
{
    ...
    HRESULT hRes = ((IDWriteTextLayout*)m_pTextLayout)->GetDrawingEffect( iPosition, &pBrush, (DWRITE_TEXT_RANGE*)outRange );
    DebugAssert( hRes == S_OK );
    ...
    hD2D1Desc.opacity = pColorBrush->GetOpacity();
    pColorBrush->GetTransform( &(hD2D1Desc.transform) );
    outBrush->m_hBrushDesc.ConvertFrom( &hD2D1Desc );

    outBrush->m_pBrush = NULL;
    hRes = pColorBrush->QueryInterface( __uuidof(ID2D1Brush), &(outBrush->m_pBrush) );
...
Void D2D1TextLayout::SetForegroundBrush( D2D1SolidColorBrush * pBrush, const D2D1TextRange * pRange )
{
    ...
    HRESULT hRes = ((IDWriteTextLayout*)m_pTextLayout)->SetDrawingEffect( (ID2D1SolidColorBrush*)(pBrush->m_pSolidColorBrush), *((const DWRITE_TEXT_RANGE *)pRange) );
    DebugAssert( hRes == S_OK );
}
...
D2D1TextInlineObject * D2D1TextLayout::GetInlineObject( UInt iPosition, D2D1TextRange * outRange ) const
{
    ...
    HRESULT hRes = ((IDWriteTextLayout*)m_pTextLayout)->GetInlineObject( iPosition, &pInlineObject, (DWRITE_TEXT_RANGE*)outRange );
    DebugAssert( hRes == S_OK );
...
Void D2D1TextLayout::SetInlineObject( D2D1TextInlineObject * pInlineObject, const D2D1TextRange * pRange )
{
    ...
    HRESULT hRes = ((IDWriteTextLayout*)m_pTextLayout)->SetInlineObject( pInlineObject->m_pTextInlineObject, *((const DWRITE_TEXT_RANGE *)pRange) );
    DebugAssert( hRes == S_OK );
}
