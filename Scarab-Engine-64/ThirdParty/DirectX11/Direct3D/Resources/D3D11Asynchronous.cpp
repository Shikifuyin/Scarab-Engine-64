...
UInt D3D11Asynchronous::GetDataSize() const
{
    ...
    return ((ID3D11Asynchronous*)m_pAsynchronous)->GetDataSize();
}
...
Void D3D11Query::_NakedCreate()
{
    ...
    hD3D11Desc.Query = (D3D11_QUERY)( D3D11QueryTypeToD3D11[m_iQueryType] );
    ...
    m_pAsynchronous = NULL;
    hRes = ((ID3D11Query*)m_pQuery)->QueryInterface( __uuidof(ID3D11Asynchronous), &m_pAsynchronous );
    DebugAssert( hRes == S_OK && m_pAsynchronous != NULL );
}
Void D3D11Query::_NakedDestroy()
{
    ((ID3D11Asynchronous*)m_pAsynchronous)->Release();
    m_pAsynchronous = NULL;

    ((ID3D11Query*)m_pQuery)->Release();
    m_pQuery = NULL;
}
...
Bool D3D11QueryCommandProcessing::Update( D3D11DeferredContext * pContext, Bool bFlush )
{
    ...
    BOOL bResult = FALSE;
    HRESULT hRes = pDeviceContext->GetData( (ID3D11Query*)m_pQuery, &bResult, sizeof(BOOL), iFlags );
    if ( hRes == S_OK ) {
...
D3D11QueryOcclusion::D3D11QueryOcclusion( D3D11Renderer * pRenderer, UInt64 * pVisibleSampleCount ):
    D3D11Query( pRenderer )
{
    m_iQueryType = D3D11QUERY_OCCLUSION;
    m_iMiscFlags = 0;
...
Bool D3D11QueryOcclusion::Update( D3D11DeferredContext * pContext, Bool bFlush )
{
    ...
    UINT64 iResult = 0;
    HRESULT hRes = pDeviceContext->GetData( (ID3D11Query*)m_pQuery, &iResult, sizeof(UINT64), iFlags );
    if ( hRes == S_OK ) {
...
D3D11QueryTimeStampFrequency::D3D11QueryTimeStampFrequency( D3D11Renderer * pRenderer, UInt64 * pFrequency, Bool * pReliableTimeStamps ):
    D3D11Query( pRenderer )
{
    m_iQueryType = D3D11QUERY_TIMESTAMP_FREQUENCY;
    m_iMiscFlags = 0;
...
Bool D3D11QueryTimeStampFrequency::Update( D3D11DeferredContext * pContext, Bool bFlush )
{
    ...
    HRESULT hRes = pDeviceContext->GetData( (ID3D11Query*)m_pQuery, &hResult, sizeof(D3D11_QUERY_DATA_TIMESTAMP_DISJOINT), iFlags );
    if ( hRes == S_OK ) {
...
D3D11QueryTimeStamp::D3D11QueryTimeStamp( D3D11Renderer * pRenderer, UInt64 * pTimeStamp ):
    D3D11Query( pRenderer )
{
    m_iQueryType = D3D11QUERY_TIMESTAMP;
    m_iMiscFlags = 0;
...
D3D11QueryStatsPipeline::D3D11QueryStatsPipeline( D3D11Renderer * pRenderer, UInt64 * pIAVertexCount, UInt64 * pIAPrimitiveCount, UInt64 * pVSInvocationCount, UInt64 * pGSInvocationCount, UInt64 * pGSPrimitiveCount,
    ...
    D3D11Query( pRenderer )
{
    m_iQueryType = D3D11QUERY_STATS_PIPELINE;
    m_iMiscFlags = 0;
...
Bool D3D11QueryStatsPipeline::Update( D3D11DeferredContext * pContext, Bool bFlush )
{
    ...
    HRESULT hRes = pDeviceContext->GetData( (ID3D11Query*)m_pQuery, &hResult, sizeof(D3D11_QUERY_DATA_PIPELINE_STATISTICS), iFlags );
    if ( hRes == S_OK ) {
    ...
    D3D11Query( pRenderer )
{
    if ( iStream == INVALID_OFFSET )
        m_iQueryType = D3D11QUERY_STATS_STREAMOUTPUT;
    else {
        DebugAssert( iStream < D3D11_SO_BUFFER_SLOT_COUNT );
        m_iQueryType = (D3D11QueryType)( D3D11QUERY_STATS_STREAMOUTPUT_0 + iStream );
    }
    m_iMiscFlags = 0;
...
Bool D3D11QueryStatsStreamOutput::Update( D3D11DeferredContext * pContext, Bool bFlush )
{
    ...
    HRESULT hRes = pDeviceContext->GetData( (ID3D11Query*)m_pQuery, &hResult, sizeof(D3D11_QUERY_DATA_SO_STATISTICS), iFlags );
    if ( hRes == S_OK ) {
    ...
    D3D11Asynchronous( pRenderer )
{
    ...
    m_iPredicateType = D3D11PREDICATE_COUNT;
    m_iMiscFlags = 0;
...
{
    D3D11_QUERY_DESC hD3D11Desc;
    hD3D11Desc.Query = (D3D11_QUERY)( D3D11PredicateTypeToD3D11[m_iPredicateType] );
    hD3D11Desc.MiscFlags = m_iMiscFlags;
    ...
    m_pAsynchronous = NULL;
    hRes = ((ID3D11Predicate*)m_pPredicate)->QueryInterface( __uuidof(ID3D11Asynchronous), &m_pAsynchronous );
    DebugAssert( hRes == S_OK && m_pAsynchronous != NULL );
}
Void D3D11Predicate::_NakedDestroy()
{
    ...
    ((ID3D11Predicate*)m_pPredicate)->Release();
    m_pPredicate = NULL;
}
...
{
    m_iPredicateType = D3D11PREDICATE_OCCLUSION;
    m_iMiscFlags = 0;
    if ( bDrawWhenUnsure )
        m_iMiscFlags |= D3D11_QUERY_MISC_PREDICATEHINT;
...
{
    return ( (m_iMiscFlags & D3D11_QUERY_MISC_PREDICATEHINT) != 0 );
}
...
Bool D3D11PredicateOcclusion::Update( D3D11DeferredContext * pContext, Bool bFlush )
{
    ...
    BOOL bResult = FALSE;
    HRESULT hRes = pDeviceContext->GetData( (ID3D11Predicate*)m_pPredicate, &bResult, sizeof(BOOL), iFlags );
    if ( hRes == S_OK ) {
...
{
    if ( iStream == INVALID_OFFSET )
        m_iPredicateType = D3D11PREDICATE_OVERFLOW_STREAMOUTPUT;
    else {
        DebugAssert( iStream < D3D11_SO_BUFFER_SLOT_COUNT );
        m_iPredicateType = (D3D11PredicateType)( D3D11PREDICATE_OVERFLOW_STREAMOUTPUT_0 + iStream );
    }
    m_iMiscFlags = 0;
    ...
    D3D11Asynchronous( pRenderer )
{
    ...
    D3D11_COUNTER_DESC hD3D11Desc;
    hD3D11Desc.Counter = (D3D11_COUNTER)( D3D11_COUNTER_DEVICE_DEPENDENT_0 + iCounterID );
...
Bool D3D11Counter::Update( D3D11DeferredContext * pContext, Bool bFlush )
{
    ...
    switch( *m_pType ) {
        case D3D11COUNTER_FLOAT: {
                FLOAT fResult = 0.0f;
                HRESULT hRes = pDeviceContext->GetData( (ID3D11Counter*)m_pCounter, &fResult, sizeof(FLOAT), iFlags );
                if ( hRes == S_OK ) {
        ...
        case D3D11COUNTER_WORD: {
                USHORT iResult = 0;
                HRESULT hRes = pDeviceContext->GetData( (ID3D11Counter*)m_pCounter, &iResult, sizeof(USHORT), iFlags );
                if ( hRes == S_OK ) {
        ...
        case D3D11COUNTER_DWORD: {
                ULONG iResult = 0;
                HRESULT hRes = pDeviceContext->GetData( (ID3D11Counter*)m_pCounter, &iResult, sizeof(ULONG), iFlags );
                if ( hRes == S_OK ) {
        ...
        case D3D11COUNTER_QWORD: {
                UINT64 iResult = 0;
                HRESULT hRes = pDeviceContext->GetData( (ID3D11Counter*)m_pCounter, &iResult, sizeof(UINT64), iFlags );
                if ( hRes == S_OK ) {
...
{
    D3D11_COUNTER_DESC hD3D11Desc;
    hD3D11Desc.Counter = (D3D11_COUNTER)( m_iCounterID );
    hD3D11Desc.MiscFlags = m_iMiscFlags;
    ...
    m_pAsynchronous = NULL;
    hRes = ((ID3D11Counter*)m_pCounter)->QueryInterface( __uuidof(ID3D11Asynchronous), &m_pAsynchronous );
    DebugAssert( hRes == S_OK && m_pAsynchronous != NULL );
}
Void D3D11Counter::_NakedDestroy()
{
    ...
    ((ID3D11Counter*)m_pCounter)->Release();
    m_pCounter = NULL;
}
