...
D3D11CommandList::~D3D11CommandList()
{
    if ( m_pCommandList != NULL ) {
        ((ID3D11CommandList*)m_pCommandList)->Release();
        m_pCommandList = NULL;
    }
}
...
Void D3D11DeferredContext::MakeCommandList( D3D11CommandList * outCommandList, Bool bRestoreDeferredContextState )
{
    ...
    HRESULT hRes = ((ID3D11DeviceContext*)m_pDeferredContext)->FinishCommandList( (bRestoreDeferredContextState) ? TRUE : FALSE, &pCommandList );
...
Void D3D11DeferredContext::Clear()
{
    ...
    ((ID3D11DeviceContext*)m_pDeferredContext)->ClearState();
}
Void D3D11DeferredContext::Reset()
{
    ...
    ID3D11CommandList * pCommandList = NULL;
    HRESULT hRes = ((ID3D11DeviceContext*)m_pDeferredContext)->FinishCommandList( FALSE, &pCommandList );
    DebugAssert( hRes == S_OK && pCommandList != NULL );
...
{
    ((ID3D11DeviceContext*)m_pDeferredContext)->Release();
    m_pDeferredContext = NULL;
}
