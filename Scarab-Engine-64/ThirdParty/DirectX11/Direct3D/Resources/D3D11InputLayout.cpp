...
Void D3D11InputLayout::PushField( const D3D11InputFieldDesc * pFieldDesc )
{
    ...
    MemCopy( m_arrFields + m_iFieldCount, pFieldDesc, sizeof(D3D11InputFieldDesc) );
    ++m_iFieldCount;
}
...
{
    D3D11_INPUT_ELEMENT_DESC arrTemp[D3D11INPUTLAYOUT_MAX_FIELDS];
    for ( UInt i = 0; i < m_iFieldCount; ++i )
...
{
    ((ID3D11InputLayout*)m_pInputLayout)->Release();
    m_pInputLayout = NULL;
}
