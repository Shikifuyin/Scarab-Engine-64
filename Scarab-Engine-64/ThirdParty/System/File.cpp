...
Bool HFile::ReadBytes( Byte * pOutBuffer, UInt iByteSize ) const
{
    ...
    DWord dwBytesRead = 0;
    BOOL bRes = ReadFile( m_hFile, pOutBuffer, (DWord)iByteSize, (LPDWORD)&dwBytesRead, NULL );
    if ( bRes == FALSE )
        return false;
...
Bool HFile::WriteBytes( const Byte * pInBuffer, UInt iByteSize ) const
{
    ...
    BOOL bRes = WriteFile( m_hFile, pInBuffer, (DWord)iByteSize, (LPDWORD)&dwBytesWritten, NULL );
    if ( bRes == FALSE )
        return false;
...
Bool HFile::ReadChar( GChar * outCh ) const
{
    ...
    DWord dwBytesRead = 0;
    BOOL bRes = ReadFile( m_hFile, &byteValue, 1, (LPDWORD)&dwBytesRead, NULL );
    if ( bRes == FALSE )
        return false;
    if ( dwBytesRead != 1 )
        return false;
...
Bool HFile::WriteChar( GChar ch ) const
{
    ...
    DWord dwBytesWritten = 0;
    BOOL bRes = WriteFile( m_hFile, &byteValue, 1, (LPDWORD)&dwBytesWritten, NULL );
    if ( bRes == FALSE )
        return false;
    if ( dwBytesWritten != 1 )
        return false;
