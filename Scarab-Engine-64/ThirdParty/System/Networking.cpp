...
HSocket::HSocket( SocketProtocol iProtocol )
{
    ...
    switch( iProtocol ) {
        case SOCKET_ICMPv4:  m_hSocket = (Void*)socket( AF_INET, SOCK_RAW, IPPROTO_ICMP ); break;
        case SOCKET_IGMPv4:  m_hSocket = (Void*)socket( AF_INET, SOCK_RAW, IPPROTO_IGMP ); break;
        case SOCKET_UDPIPv4: m_hSocket = (Void*)socket( AF_INET, SOCK_DGRAM, IPPROTO_UDP ); break;
        case SOCKET_TCPIPv4: m_hSocket = (Void*)socket( AF_INET, SOCK_STREAM, IPPROTO_TCP ); break;
        case SOCKET_ICMPv6:  m_hSocket = (Void*)socket( AF_INET6, SOCK_RAW, IPPROTO_ICMPV6 ); break;
        case SOCKET_IGMPv6:  m_hSocket = (Void*)socket( AF_INET6, SOCK_RAW, IPPROTO_IGMP ); break;
        case SOCKET_UDPIPv6: m_hSocket = (Void*)socket( AF_INET6, SOCK_DGRAM, IPPROTO_UDP ); break;
        case SOCKET_TCPIPv6: m_hSocket = (Void*)socket( AF_INET6, SOCK_STREAM, IPPROTO_TCP ); break;
...
{
    Int iRes = shutdown( (SOCKET)m_hSocket, (Int)iType );
    DebugAssert( iRes == 0 );
}
Void HSocket::Close()
{
    Int iRes = closesocket( (SOCKET)m_hSocket );
    DebugAssert( iRes == 0 );
...
NetworkError HSocket::GetOption( SocketOption iOption, Void * outValue ) const
{
    ...
    Int iResult = getsockopt( (SOCKET)m_hSocket, iLevel, iOptionName, (char*)outValue, &iOptionLength );
    if ( iResult == 0 )
        return NETWORK_OK;
...
NetworkError HSocket::SetOption( SocketOption iOption, const Void * pValue ) const
{
    ...
    Int iResult = setsockopt( (SOCKET)m_hSocket, iLevel, iOptionName, (const char*)pValue, iOptionLength );
    if ( iResult == 0 )
        return NETWORK_OK;
...
Void HSocket::SetMode( Bool bNonBlocking ) const
{
    ...
    Int iRes = ioctlsocket( (SOCKET)m_hSocket, FIONBIO, (u_long*)&ulParam );
    DebugAssert( iRes == 0 );
}
Void HSocket::GetPendingData( ULong * outPendingBytes ) const
{
    Int iRes = ioctlsocket( (SOCKET)m_hSocket, FIONREAD, (u_long*)outPendingBytes );
    DebugAssert( iRes == 0 );
}
...
NetworkError HSocket::Connect( const NetAddress * pRemoteAddress ) const
{
    ...
    switch( pRemoteAddress->iType ) {
        case NETADDRESS_IPv4: {
                ...
                iResult = connect( (SOCKET)m_hSocket, (const SOCKADDR *)( &sockIPv4 ), sizeof(SOCKADDR_IN) );
            } break;
        case NETADDRESS_IPv6: {
                ...
                iResult = connect( (SOCKET)m_hSocket, (const SOCKADDR*)( &sockIPv6 ), sizeof(SOCKADDR_IN6) );
            } break;
        default: DebugAssert( false ); break;
    }
    if ( iResult == 0 )
...
NetworkError HSocket::Bind( const NetAddress * pLocalAddress ) const
{
    ...
    switch( pLocalAddress->iType ) {
        case NETADDRESS_IPv4: {
                ...
                iResult = bind( (SOCKET)m_hSocket, (const SOCKADDR *)( &sockIPv4 ), sizeof(SOCKADDR_IN) );
            } break;
        case NETADDRESS_IPv6: {
                ...
                iResult = bind( (SOCKET)m_hSocket, (const SOCKADDR*)( &sockIPv6 ), sizeof(SOCKADDR_IN6) );
            } break;
        default: DebugAssert( false ); break;
    }
    if ( iResult == 0 )
...
NetworkError HSocket::Listen( UInt iMaxPending ) const
{
    ...
    Int iResult = listen( (SOCKET)m_hSocket, (Int)iMaxPending );
    if ( iResult == 0 )
        return NETWORK_OK;
...
NetworkError HSocket::Accept( HSocket * outClientSocket, NetAddress * outClientAddress ) const
{
    if ( outClientAddress != NULL ) {
        ...
        outClientSocket->m_hSocket = (Void*)accept( (SOCKET)m_hSocket, &sockAddr, &iAddrLength );
        ...
        outClientSocket->m_hSocket = (Void*)accept( (SOCKET)m_hSocket, NULL, NULL );

    if ( outClientSocket->m_hSocket == (Void*)INVALID_SOCKET ) {
...
{
    Int iResult = send( (SOCKET)m_hSocket, (const char *)streamData, (Int)iLength, 0 );
    if ( iResult == SOCKET_ERROR ) {
        iResult = WSAGetLastError();
        switch(iResult) {
            case WSAEACCES:       return NETWORK_NO_BROADCAST;
...
{
    Int iResult = recv( (SOCKET)m_hSocket, (char*)streamData, (Int)iLength, 0 );
    if ( iResult == SOCKET_ERROR ) {
        iResult = WSAGetLastError();
        switch(iResult) {
            case WSAETIMEDOUT:    return NETWORK_CONNECT_TIMEOUT;
...
NetworkError HSocket::SendTo( const NetAddress * pRemoteAddress, const Byte * packetData, UInt iLength, UInt * outBytesSent ) const
{
    ...
    switch( pRemoteAddress->iType ) {
        case NETADDRESS_IPv4: {
                ...
                iResult = sendto( (SOCKET)m_hSocket, (const char *)packetData, (Int)iLength, 0, (const SOCKADDR *)( &sockIPv4 ), sizeof(SOCKADDR_IN) );
            } break;
        case NETADDRESS_IPv6: {
                const NetAddressIPv6 * pAddr = (const NetAddressIPv6 *)pRemoteAddress;
                ...
                iResult = sendto( (SOCKET)m_hSocket, (const char *)packetData, (Int)iLength, 0, (const SOCKADDR *)( &sockIPv6 ), sizeof(SOCKADDR_IN6) );
            } break;
        default: DebugAssert( false ); break;
    }
...
NetworkError HSocket::ReceiveFrom( NetAddress * outRemoteAddress, Byte * packetData, UInt iLength, UInt * outBytesReceived ) const
{
    ...
    Int iResult = recvfrom( (SOCKET)m_hSocket, (char*)packetData, (Int)iLength, 0, &sockAddr, &iAddrLength );
