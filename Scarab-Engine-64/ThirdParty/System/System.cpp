...
Void _RevertTimeDate( SYSTEMTIME * outSystemTime, const TimeDate * pTimeDate )
{
...
TimeValue _ConvertTimeValue( const FILETIME * pFlatTime, TimeUnit iResolution )
{
    ULARGE_INTEGER iTmp;
...
Void _RevertTimeValue( FILETIME * outFlatTime, TimeValue iTimeValue, TimeUnit iResolution )
{
    ULARGE_INTEGER iTmp;
    switch ( iResolution ) {
...
Void _ConvertTimeZone( TimeZone * outTimeZone, const TIME_ZONE_INFORMATION * pTimeZoneInfos )
{
...
Void _RevertTimeZone( TIME_ZONE_INFORMATION * outTimeZoneInfos, const TimeZone * pTimeZone )
{
...
UInt System::ConsoleRead( GChar * outBuffer, UInt iMaxLength ) const
{
    ...
    DWord iRead = 0;
    BOOL bOk = ReadConsole( hIn, outBuffer, iMaxLength, (LPDWORD)&iRead, NULL );
...
Void System::ConsoleWrite( const GChar * strText, UInt iLength ) const
{
    ...
    DWord iWritten = 0;
    BOOL bOk = WriteConsole( hOut, strText, iLength, (LPDWORD)&iWritten, NULL );
...
Bool System::DestroyDirectory( const GChar * strPathName )
{
    ...
    while( bContinue ) {
        ...
        strPtr = StringFn->NCopy( strPtr, TEXT("/"), PATHNAME_LENGTH - (UInt)(strPtr - strSubPath) );
        strPtr = StringFn->NCopy( strPtr, strName, PATHNAME_LENGTH - (UInt)(strPtr - strSubPath) );
...
Bool System::ListDirectoryFirst( const GChar * strPathName, Bool * outIsSubDirectory, GChar * outFileName, UInt iMaxLength )
{
    ...
    strPtr = StringFn->NCopy( strPtr, TEXT("/*"), PATHNAME_LENGTH + 2 - (UInt)(strPtr - strSearch) );
...
Void System::GetMACAddress( Byte outMACAddress[8] ) const
{
    ...
    ULong ulRes = GetAdaptersAddresses( AF_INET, 0, NULL, pAddresses, (PULONG)&iLength );
    if ( ulRes == ERROR_BUFFER_OVERFLOW ) {
        _ScratchFree( pAddresses );

        pAddresses = (IP_ADAPTER_ADDRESSES*)( _ScratchAlloc(iLength) );
        ulRes = GetAdaptersAddresses( AF_INET, 0, NULL, pAddresses, (PULONG)&iLength );
    }
...
Void System::StringToNetAddress( NetAddress * outAddress, const GChar * strDottedFormat, NetAddressType iType ) const
{
    switch( iType ) {
        case NETADDRESS_IPv4: {
                ...
                GChar strAddr[16];
                StringFn->NCopy( strAddr, strDottedFormat, (UInt)(strPort - strDottedFormat) );
                ++strPort;
                                      ...
                                      SocketProtocol protocolHint, AddressResolveFlags iFlags )
{
    ...
    Int iResult = GetAddrInfo( strHostName, strServiceName, &hints, (ADDRINFOT**)&m_pCurAddrInfo );
    if ( iResult == 0 )
        return NETWORK_OK;
...
Bool System::EnumResolvedNetAddresses( NetAddress * outAddress, SocketProtocol * outProtocol )
{
    ...
    *outProtocol = HSocket::_BuildSocketProtocol( ((ADDRINFOT*)m_pCurAddrInfo)->ai_socktype,
                                                  ((ADDRINFOT*)m_pCurAddrInfo)->ai_family,
                                                  ((ADDRINFOT*)m_pCurAddrInfo)->ai_protocol );

    if ( (*outProtocol) & 0x80000000 ) {
        NetAddressIPv6 * outIPv6 = (NetAddressIPv6*)outAddress;
        outIPv6->iType = NETADDRESS_IPv6;

        SOCKADDR_IN6 * pAddr = (SOCKADDR_IN6*)(((ADDRINFOT*)m_pCurAddrInfo)->ai_addr );
        ...
        SOCKADDR_IN * pAddr = (SOCKADDR_IN*)(((ADDRINFOT*)m_pCurAddrInfo)->ai_addr );
        outIPv4->Addr.Value = pAddr->sin_addr.s_addr;
        outIPv4->wPort = pAddr->sin_port;
    }

    m_pCurAddrInfo = ((ADDRINFOT*)m_pCurAddrInfo)->ai_next;
