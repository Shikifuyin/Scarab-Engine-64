...
{
    DWord dwExitCode = 0;
    BOOL bRes = GetExitCodeProcess( m_hThreadingObject, (LPDWORD)&dwExitCode );
    DebugAssert( bRes != FALSE );
    return dwExitCode;
}
    ...
    HThreadingObject()
{
    m_hThreadingObject = CreateThread( NULL, 0, pfEntryPoint, pParameters,
                                       bRun ? 0 : CREATE_SUSPENDED, (LPDWORD)outID );
    DebugAssert( m_hThreadingObject != NULL );
}
...
{
    DWord dwExitCode = 0;
    BOOL bRes = GetExitCodeThread( m_hThreadingObject, (LPDWORD)&dwExitCode );
    DebugAssert( bRes != FALSE );
    return dwExitCode;
}
